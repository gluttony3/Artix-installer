...
install_base() {
    ...
    echo ""
    echo -e "${YELLOW}  NOTE: basestrap may ask you to choose between package providers.${NC}"
    echo -e "  Recommended choices:"
    echo -e "    libxtables       -> ${BOLD}iptables-nft${NC} (modern nftables backend)"
    echo -e "    initramfs        -> ${BOLD}mkinitcpio${NC}   (default, well-supported)"
    echo -e "    x11win-server    -> ${BOLD}xorg-server${NC}  (required for xf86 drivers)"
    echo -e "    qt6-multimedia   -> ${BOLD}qt6-multimedia-ffmpeg${NC}"
    echo ""
